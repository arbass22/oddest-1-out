...
const Modal: React.FC<ModalProps> = ({ title, message, buttonText, onAction, colorClass }) => (
  <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div className="bg-white rounded-lg shadow-2xl max-w-md w-full p-8 text-center animate-in fade-in zoom-in duration-300">
      ...
      <p className="text-stone-600 mb-8 leading-relaxed whitespace-pre-wrap">{message}</p>
      <button
        onClick={onAction}
        className="bg-stone-900 text-white px-8 py-3 rounded-full font-bold hover:bg-stone-700 transition-colors uppercase tracking-widest text-sm"
      >
        {buttonText}
      </button>
    </div>
